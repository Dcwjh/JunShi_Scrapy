...
import json
from military_crawler.items import WikiTripleId, WikiTripleItem, ZhWikiKeyword, WikiTableTriple, Triple, NewTriple
import zhconv
...
class MilitaryCrawlerPipeline(object):
    ...
    def open_spider(self, spider):
        self.dict_files = {}
        for path in spider.category_names:
            self.dict_files[path] = open(path + ".csv", 'a', encoding='utf-8')
    ...
    def process_item(self, item, spider):
        if isinstance(item, ZhWikiKeyword):
            if item['label'] in self.dict_files:
                line = zhconv.convert(item['title'], 'zh-hans') + ',' + item['url'] + '\n'
                self.dict_files[item['label']].write(line)
        ...
        elif isinstance(item, NewTriple):
            if item['label'] in self.dict_files:
